...
**Current state (2026-09-12) — M3 built, awaiting the M3 checkpoint**

- Everything from M2 plus the M3 build list (§20): the knife, handgun and rifle with an over-the-shoulder aim (hold right mouse, a ring on whoever is under the crosshair, left mouse fires or stabs; rounds and reloads per §12.1); the confrontation rules (§12.2) — weapons never fire at an Innocent (the trigger stops short and they react: unarmed people put their hands up and the robbery dialogue opens; armed people draw and fight back), thieves are Threatening from their first step (an unarmed one surrenders and hands the loot back, an armed one fights), hostiles approach, attack with bad aim and run at one hit point, and poof at zero in a red cloud leaving a loot bag with everything they carried (stolen items included) and a floating hat; the hidden wanted value and the Park Ranger (Rhonda, or Tom when she was poofed, both once a ranger was poofed) who lectures, confiscates the best weapon plus one item as a fine and resets wanted; the rowboat; the Map screen (M); achievement dates and progress in the Journal; every achievement wired. Evidence and known gaps: §20 M3 status.
- **Play it:** `npm run dev`. Fish up (or `give`) a weapon, select it, hold right mouse to aim at someone. `event ranger` forces the ranger; `wanted 2` + `director on` + `director now` lets the Director send her. The boat waits beside the dock (F to board).
- The M3 checkpoint asks are in §22.

**Previous state (2026-09-11) — M2 built, awaiting the M2 checkpoint**
...
| GPUs | Intel Iris Xe (Raptor Lake-P) + NVIDIA RTX 4050 Max-Q (hybrid laptop); in NVIDIA mode since 2026-09-12 (`system76-power graphics`) | Performance targets in §17. Flatpak Chromium only ever sees the Iris Xe (Mesa); `npm run perf:nvidia` measures the RTX. |
| Brave | 151.1.93.129, native (`/opt/brave.com/brave/brave`) | The owner's browser (Wayland, COSMIC). `tools/perf-nvidia.sh` drives it headless with a throwaway profile. |
...
2. The M1 and M2 checkpoints passed (§1 #31; M2 nine rounds of notes, §1 #32–45, all fixed and committed). The owner said "start M3" on 2026-09-12; **M3 is built and its checkpoint is in progress** (§20 M3 status, §22 M3 asks). Do not start M4 until the owner is satisfied. Still open from M2: the alien interview (§22 #6). Record feedback in §1 / §22 first.
...
- 2026-09-11 — Round 7 committed (`8ae9b27`). Round 8 (§1 #43): character separation in the NPC manager, visitors keep their own spots and leave at different paces.
- 2026-09-11 — Rounds 4–6 committed (`f5d5fee`). Round 7 (§1 #41–42): messages in bottles built (moved up from M3; twelve notes, Journal cards, smoke step); the swimsuit garment is now a real one-piece (CC-BY asset from the underwear03 pack, §4.5 asset table, §23).
- 2026-09-12 — Round 8 committed (`ded60ed`). Round 9 (§1 #44): bank-aware routing and spawn nodes, NPC stuck tracking, visit give-up timeouts (`events.visitApproachTimeoutSeconds`; the stuck/leave timeouts were generalised the same day, next entry).
- 2026-09-12 — Round 9 committed (`d928d7a`). Implementer hardening, no owner note: the round-9 review found the party, grudge and water-walker leave phases had no stuck or timeout handling (a stuck leaver would hold the single event slot forever, the round-9 failure in another runner). Every runner now ends when an NPC is stuck: `events.stuckSeconds` (8) and `events.leaveTimeoutSeconds` (120) replace the visit-only `visitStuckSeconds`/`visitLeaveTimeoutSeconds`; a stuck partier dances on the spot, a stuck grudge storms off, a stuck water-walker says her bit from where she stands, stuck or late leavers of every event go home directly; partiers leave at staggered paces (no clump); a sneaking thief never spawns across the river; thieves and grudges leave along their own bank; a conversation stops a walking NPC where they stand (they used to keep walking and lose the talking clip on arrival); `Npc.goTo` resets the stuck counter so a new route gets its own chance. Verified by a headless probe that pins one NPC per runner (party, grudge leave + approach, water-walker approach + leave, talk mid-approach, thief spawn side) and the full smoke. Committed (`dc2f30b`).
- 2026-09-12 — Owner said "start M3". M3 built (§0.1 current state, §20 M3 status): weapons and the aim camera, the confrontation rules as a pure sim (`src/sim/confrontation.ts`, tested) presented by `src/gameplay/combat.ts` (hands-up robbery dialogue, thief surrender, armed fight-back, hostile AI, the red poof with a loot bag and a floating hat, animal scaring through the runners' `threatTarget()`), the wanted value and the Park Ranger runner, the rowboat (`src/sim/boat.ts` + `src/actors/boat.ts`, boarding at the dock or a bank, rowing, fishing from it, the gator's in-boat rule, saved position, back to the dock on death), the Map screen, achievement dates and progress in the Journal, every achievement wired with a unit-tested unlock path (`tests/achievements.test.ts`). Implementer decisions recorded in §12 (M3 implementation notes) and §7.1 (the dock shortened to mid-channel). No new dependencies; save schema stays at 2 with optional additions (§14.2).
- 2026-09-13 — M3 committed (`68435f4`) at the owner's request.
- 2026-09-12 — Owner: the [ ] hotbar keys are a fine alternative to the wheel (§1 #40 resolved, §22 (f) closed). The owner switched the laptop to NVIDIA mode (`system76-power graphics` → nvidia); perf re-measured on the RTX 4050 (§17): 60 fps everywhere on Medium and High, party included. Added `npm run perf:nvidia` (`tools/perf-nvidia.sh`, a `CHROMIUM_ARGS` hook in `tools/cdp.mjs`) because the Chromium flatpak can't reach the NVIDIA driver (§19). The undershirt ask (§22 (g)) closed: no undershirt. A cropped tank top (`toigo_camisole_top`) and then a short camisole (`elvs_camisole1` cut at the waist by a generator crop option) were built in place of the bra and both reverted the same session — §1 #26 stands, no code or asset change remains. Dialogue tone accepted (§1 #45, §22 #7). Open ask left: the alien interview.
...
| 26 | Underwear base layer | Male: **white boxers with red spots** (a polka-dot texture on the shorts, not plain white). Female: **white bra + white boy shorts cut like the male's M0 shorts** (`cortu_jeans_shorts` recoloured), not panties. *(2026-09-12: stands as written. The owner declined an undershirt for the man, then tried a cropped tank top and a short camisole in place of the bra and reverted both — the bra stays.)* |
...
| 40 | Mouse wheel (rounds 4–6) | Round 4: "mostly works now, but sometimes doesn't"; round 5: it works with the camera in front of the character and stops with the camera behind, no Escape or E involved. **Cause (2026-09-11):** the owner plays in Brave as a native Wayland client on COSMIC. Chromium on Wayland never re-centres the pointer while it is locked (every other platform warps it back near the window border), so the browser's idea of where the hidden pointer is drifts with every mouse movement — turning the camera *is* that drift — and wheel events, hit-tested at that position, are dropped before the page sees them once it has left the window. The round-4 "Escape exit" explanation was wrong (that gate stays removed; harmless). **Tried and reverted (round 6):** a page-side resync that released and re-took the lock whenever the estimate left the window. Chromium shows the "To show your cursor, press Esc" banner on *every* lock acquisition, so it was on screen constantly; a refused re-lock opened the pause menu ("it still loses focus"); and the lock churn plus a timer armed on every mouse event produced a walking jitter ("double vision"). Rule: **lock the pointer once per play session, never re-lock on the page's own initiative.** What ships: **[ and ] cycle the hotbar** (rebindable `slotPrev`/`slotNext`) and the console command `input` (captured/locked state, viewport, wheel-event count, lock-change count). Owner-side check that would confirm the cause and restore the wheel: Brave → `brave://flags/#ozone-platform-hint` → X11 (XWayland re-centres the pointer under lock). Not reproducible here (no Wayland input injection). **Resolved 2026-09-12:** "[ ] hotbar keys are a fine alternative." The wheel stays as it is; no further work. |
| 41 | Messages in bottles (round 7, 2026-09-11) | "I have received multiple messages in a bottle but none of them have shown up in my journal." The item, the Journal tab and the *Castaway* check existed, but nothing ever wrote a note into the Journal and no note texts existed (it was on the M3 list). Built now: twelve notes in `src/data/messages.ts`; fishing up a Message in a Bottle reads one unread note into the Journal (toast says which of twelve), the Journal's Messages tab shows the texts as cards, and once all twelve are found further bottles only repeat. The bottle stays in the inventory as junk. |
| 42 | Marina's swimsuit (round 7) | "Marina Costa (the woman who comes out of the water) should be wearing a normal women's one piece swimsuit." The `swimsuit` garment was MakeHuman's `female_sportsuit01` (a sports outfit). Replaced by `mindfront_f_one-piece_swimsuit_01` by Mindfront (CC-BY, MakeHuman underwear03 pack, `mpfb-pack-underwear03` in the manifest and ASSETS.md) on both bodies, so Sierra Blake and the fished-up Swimsuit item change with it. |
| 43 | Fused visitors (round 8, 2026-09-11) | "When two people approach, after I interact with them, they have a tendency to coalesce into a single, freakish person… As they leave, they are still pasted together, step for step." Causes: the "follow once" re-approach sent every visitor to the first one's exact spot; a pair left on the identical path at the identical speed; and near the water the land clamp folded both approach points onto one bank point. Fixes: **characters never share a spot** — the NPC manager resolves overlaps every tick (`src/sim/separation.ts`, `npc.minSeparation` 0.9 m; someone rising from the water or mid-conversation is not shoved), and a walker whose destination is already occupied counts as arrived instead of jostling; each visitor re-approaches to their own spot; a pair leaves at slightly different paces; the approach point clamps to land before the side step and again after. |
| 44 | Visitor stranded across the river (round 9, 2026-09-12) | "A guy showed up on the other side of the river from me. He has been standing there for many minutes… It is where the river is very wide." A visitor whose route ended at a trail node on the far bank walked straight at the water, could not enter it, and stood there; the visit only starts its clock once everyone has arrived, so it never left — and it held the one active event slot, so nothing else could happen either. Fixes: routes now start and end at the nearest trail node **on each end's own bank**, so a crossing goes over a bridge (probe: west bank → bridge → east bank of the pool, arrived in 50 s at a jog); spawn points prefer the player's bank at a looser distance before ever using the far bank; NPCs track how long they have been stuck; a visitor still walking after `visitApproachTimeoutSeconds` (60) or stuck for `visitStuckSeconds` (8) waves from where they are; a leaver leaves along their own bank's trail, and one still about after `visitLeaveTimeoutSeconds` (90) or stuck goes home directly, so the event always ends. *(2026-09-12: the stuck/leave tunables became `events.stuckSeconds` / `events.leaveTimeoutSeconds`, shared by every runner — §0.2.)* |
| 45 | Dialogue tone (2026-09-12) | "dialog is fine." All 58 trees stand as written; §22 #7 resolved. |
...
| Swimsuit (one-piece) | MakeHuman community `underwear03` pack, asset `mindfront_f_one-piece_swimsuit_01` by Mindfront, fetched by `npm run assets` as `mpfb-pack-underwear03` and fitted to both player bodies (`players.json` garment `swimsuit`, flat colour). Replaced `female_sportsuit01` on 2026-09-11 (§1 #42). | CC-BY 4.0 (credited in ASSETS.md) |
...
- **Implementation note (M3):** F boards within `boat.boardRange` (3.2 m) of the boat; F steps out when a land point (1 m up the bank) or the dock's end is within `boat.leaveRange` (5 m), otherwise a caption says to row to a bank. The hull needs `water.boatMinDepth` under its bow and stern (`boat.hullRadius`), so it follows the channel (deep along the middle of the whole river) and bumps the shallows. Rowing auto-reels the line; the rower sits with the `sit_idle` clip and the `push` clip swings the oars. Movement keys row while aboard; the camera and hotbar work as on foot (aiming from the boat included).
...
- **Dock (M3):** the river is only ~4.5 m wide at the dock's z, so the 7 m dock authored in M1 reached the far bank. `src/world/valley.ts` now caps its length to reach about the middle of the channel; the boat waits alongside its end.
...
**M3 implementation (2026-09-12).** The rules are a pure sim, `src/sim/confrontation.ts` (stances, the 1.5 s aim reaction, `attack` that blocks against an Innocent and otherwise takes the table damage off `confrontation.npcHitPoints` (2; knife 1, handgun 1, rifle 2), the trigger with rounds and reloads, hostile decisions, wanted, robbery outcomes, surrender and poof loot), tested in `tests/confrontation.test.ts`. `src/gameplay/combat.ts` presents it: the target is the person nearest the crosshair ray within `confrontation.targetRadius`; the knife hits an arc in front of the player, firearms a hitscan with `confrontation.spread`; each hostile is driven by the combat system (it "holds" the NPC — `Npc.engaged` — so its event runner cannot despawn it mid-fight; every runner has `onThreatened(npcId)`, `onNpcGone(npcId, reason)`, `onRobbed(npcId)` and `threatTarget()` hooks). Ammo: one inventory unit is one round; a fished-up box gives `ammoCount` rounds. Decisions the plan left open, taken by the implementer (the owner can overrule): thieves are Threatening from their first step (aiming at a sneaking thief stops the theft — *Not Today*); an **armed** thief aimed at fights instead of surrendering; refusing an armed grudge's demand starts a fight (the M2 punch is gone — §11.4 "or fight"); "Everything!" takes up to `confrontation.robAllMaxItems` (3) stacks; "Just kidding!" is −20 relationship and no wanted; a robbery that takes nothing (empty pockets) adds no wanted; a hostile stuck for `events.stuckSeconds` (melee) or fighting for 90 s gives up and runs (grudge kept); a gunman who cannot get closer shoots from where he stands; the ranger's fine is the most valuable unbound non-weapon item; a ranger with no weapon to take issues "a warning and a fine". The poof: 80 red particles, a cartoon pop, the hat (if any) floats up and drops; the loot bag is a pickup with `contents` (F gives everything inside). Every M3 number is in `TUNABLES.confrontation`, `TUNABLES.ranger`, `TUNABLES.boat`.
...
**M3 implementation (2026-09-12).** Every achievement has a trigger in `src/sim/achievements.ts` (or, for *Is This Yours?* and *Believer*, an `unlock` effect in the dialogue data — *Believer* also counts any talk with Carl in the hat); `tests/achievements.test.ts` proves each non-secret one unlocks from a fresh state via its console/scripted path and that no id is left without one. *Breezy*/*Dapper* accumulate worn seconds (a full in-game day = 24 real minutes; *Breezy* only for a character who didn't start in a dress, i.e. a man); *Dressed by the River* keys on garments fished up (`journal.fishedGarments`, `id:colour`); *Oarsome* on `stats.boatMetres`; *Conga!* fires when the player talks to Destiny at a party within an in-game hour of a bear (only one event runs at a time, so "nearby" means "just left"). The Journal's Achievements tab shows the unlock date and a progress bar for counters; secrets show as "???" until earned.
...
*M3 additions (still `schemaVersion: 2`; every new field is optional and defaults on load):* `world.pickups[].contents` (a loot bag's stacks), `director.rangersBoth`, `progress.journal.fishedGarments`, and the M3 stats; `player.inBoat` and `world.boat` are now written for real (the boat's position and heading; boarding is restored on load).
...
- **Screens:** Journal (fish log with silhouettes for uncaught species, people met with relationship, messages in bottles, achievements); Map (painted park map, zone names, player marker, trashed zones marked).
  - **Map (M3):** `src/ui/screens/map.ts` paints a 2D canvas from the valley data — forest wash, the river from the spline, trails, bridges, the dock, area names, zone names along the water, trashed zones as an amber blotch, the boat, and "you are here" with the facing. No render-to-texture.
...
**M2 measurements on the RTX 4050 (2026-09-12, `npm run perf:nvidia`, the owner's native Brave 151 headless on the discrete GPU via PRIME render offload, 1920×1080, `director off`):** every station at a vsync-capped 60 fps on **both** presets — Medium worst p95 17.8 ms (party: 214 draw calls, 1.46 M triangles, six NPCs); High worst p95 17.4 ms (party: 226 draw calls, 2.2 M triangles). The owner switched the laptop to NVIDIA mode that day (`system76-power graphics` → nvidia), so this is what he sees. Same day on the Intel Iris Xe (flatpak Chromium, Mesa): Medium 60 fps at all seven stations, worst p95 18.0 ms — but that party rolled fewer than six NPCs (182 draw calls), so the six-NPC 47 fps figure above still stands as the Iris Xe worst case; High on the Iris Xe is 41–49 fps at the pool, campground, bridge and party (worst p95 26.3 ms), as expected for a preset meant for discrete GPUs. Method note: the Chromium flatpak carries only Mesa GL runtimes (no `org.freedesktop.Platform.GL.nvidia-*` extension), so it can never reach the proprietary driver; and headless Chromium's Vulkan path falls back to SwiftShader on this machine, so `tools/perf-nvidia.sh` uses the ANGLE-over-GL path with `__NV_PRIME_RENDER_OFFLOAD=1`. A stray `DRI_PRIME=pci-0000_00_02_0` in the agent's shell pins Mesa to the Intel card; the script unsets it.
...
- M3 additions: `poof <npcId>`, `hostile <npcId>`, `stance <npcId> innocent|threatening|hostile`, `combat` (aim/target/stances), `boat here|dock`, `board`, `leave`; `event ranger` works; `director now` also clears the minimum gap after the last event. `window.__ss` adds `aimAt(npcId)` (points the crosshair at them), `pickupAll(radius)`, `board()`, `leave()`, and `state()` gains `combat`, `boat`, `pickupList`.
...
| `npm run smoke` | Launches `flatpak run org.chromium.Chromium` headless over CDP against the dev server. Plays a scripted path: menus → create character → walk the trail → cast → force a bite → catch → equip clothing → force each event via `__ss` hooks → save & quit → load; then (M3) the robbery of Barb, Wade's poof and loot bag, a thief's surrender and a poofed thief's returned loot, the Director-forced ranger, the boat, death keeping achievements, the Map. Screenshots every step into `smoke-out/` and **fails on any console error**. Closes Chromium via CDP (killing the flatpak wrapper leaves the browser running). |
...
| `npm run perf` | Headed Chromium with a real GPU runs a scripted flythrough + walk. Reports p50/p95 frame time, draw calls and triangles per preset. Run on the integrated GPU (Medium target) and the RTX 4050. Software rendering (SwiftShader) is only for correctness screenshots, never performance. `npm run perf:nvidia` runs the same stations on the NVIDIA GPU through the native Brave binary, headless (the flatpak Chromium can't reach the proprietary driver). |
...
**Status (2026-09-12): built; checkpoint in progress** — the owner said "start M3" and the whole build list below is in, committed 2026-09-13 (`68435f4`). Evidence: `typecheck`, `test` (**89** pass: the M2 suites plus `confrontation` — weapons never damage an Innocent or an animal, the 1.5 s reaction per stance, the trigger/rounds/reloads, hostile decisions and bad aim, wanted (+1 robbery, +0.5 fight, 0 for poofing a hostile or a thief, +2 for a ranger; the ranger forced at 2), robbery outcomes with the stolen ledger, surrender and poof loot, the ranger's pick, death loses fish only — `boat` — rowing, turning, the shallow-water bump, the seat — and `achievements` — every non-secret achievement unlocks from a fresh state through its console/scripted path, Larry's keys through the dialogue effect, achievements/loot-bag contents/`rangersBoth` survive a save round trip), `deps:check` (all ≥ 17 d), `build` (rolldown, the same 2.5 MB chunk warning as before), `simulate` (300 h: 31.7 events/h at full ramp outside lulls, 26.9 fish/h, 26% of bites lost, median time-to-Old-Gus 3.1 h — the M3 changes touch no pacing), `npm run smoke -- --gpu` **SMOKE PASSED** (the M1 path, every M2 event, then the M3 section: 45 screenshots in `smoke-out/`, zero console errors; ~49 fps on the Iris Xe at the end of the run). Acceptance: every line below is exercised by the smoke's M3 section (screenshots `36-aim-barb` … `journal-achievements`): the trigger stops short on Barb and her hands go up into the robbery (+1 wanted, a grudge, *Trailway Robbery*); Wade draws (+0.5) and his poof leaves a bag with his ammo (*Poof, There It Is*; no wanted); Candy surrenders before taking anything (*Not Today*); Pete's bag returns the trophy (*Repo Man*); a second robbery reaches wanted 2 and the Director sends Rhonda, who takes the rifle and leaves the knife (*Most Wanted*, wanted 0); the boat is boarded at the dock, rowed, fished from (*Boat Life*) and left at the dock; death keeps every achievement and loses only the fish; the Map and the Journal's dated achievements render. Known gaps carried to M4: the rifle reuses the pistol aim/shoot clips (no two-handed pose); no boat wake or ripples; hostiles have no muzzle flash of their own; the bear placeholder and the Iris Xe party budget from M2; the bundle still copies all of `public/`; no ambient passersby.
...
- **Cast & collection:** all 58 NPCs with dialogue and cross-links (Hendersons, Carl + tinfoil hat, Larry's keys…); ~~messages in bottles~~ (built in M2, §1 #41); Map screen; complete Journal; all achievements (§13) with UI and toasts.
...
7. ~~Dialogue tone review~~ — resolved 2026-09-12: "dialog is fine" (§1 #45). The 58 trees stand.
...
**M2 checkpoint, round 4 (2026-09-11):** (a) pacing is fine (§1 #37); (c) not seen yet — no abduction so far; (d) the synthesized bed is gone (§1 #38); (e) the placeholder bear stays but had to move (§1 #39); (f) the wheel was intermittent — the Escape diagnosis was wrong; round 5 pinned it to Brave on Wayland and the pointer-lock drift; the resync attempted in round 5 was reverted in round 6; [ ] keys and the `input` diagnostic ship, and the owner can confirm the cause by switching Brave to X11 (§1 #40); on 2026-09-12 the owner accepted the [ ] keys as a fine alternative, so (f) is closed; the same day he declined the undershirt ("funny to go the other way" — a one-line bottoms-only experiment for a laugh, reverted the same session), so (g) is closed and §1 #26 stands (a cropped tank top and a short camisole were tried in place of the bra the same day and both reverted); (b) dialogue tone: "dialog is fine" (§1 #45). Still open: (c) the alien interview only.

**M3 checkpoint asks (2026-09-12):** (a) the aim feel — over-the-shoulder distance and offset (`src/actors/camera.ts`), the 1.5 s reaction, spread, the hostiles' accuracy and cadence (`TUNABLES.confrontation`); (b) the robbery menu's lines and the relationship/wanted numbers (§12 implementation notes list the implementer's calls: armed thieves fight rather than surrender, refusing a grudge starts a fight, "Everything!" is three stacks); (c) the ranger — is one weapon plus one item the right fine, and should she come sooner or later than wanted 2; (d) the boat — speed, turning and where it may be left (`TUNABLES.boat`), and whether the narrow, shortened dock spot is fine or the dock should move to a wider stretch; (e) the red poof and the hat — enough of a gag, or should the poof be bigger; (f) still open from M2: the alien interview (#6).
